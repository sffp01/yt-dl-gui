...
autoUpdater.on("checking-for-update", () => {
  console.info("message", "Checking for update...");
});
...
autoUpdater.on("update-not-available", info => {
  console.info("message", `Update not available.`);
  createWindow();
});
...
autoUpdater.on("download-progress", progressObj => {
  progressBar.detail = `${progressObj.transferred} / ${progressObj.total}`;
});
...
app.on("ready", () => {
  isDev ? createWindow() : autoUpdater.checkForUpdates();
});
